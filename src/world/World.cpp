...
void    World::loadGame(std::string const &file)
{
  ...
  mapHeightName = dynamic_cast<JSON::JsonStr &>(json.GetObj(
	  "height_map")).Get();
  ...
  try
    {
      JSON::JsonObj &endZone = dynamic_cast<JSON::JsonObj &>(json.GetObj(
	      "endZone"));
      endPos.first = stod(
	      dynamic_cast<JSON::JsonStr &>(endZone.GetObj("posX")).Get());
      endPos.second = sizeY - stod(dynamic_cast<JSON::JsonStr &>(endZone.GetObj(
	      "posY")).Get());
      endSize.first = stod(
	      dynamic_cast<JSON::JsonStr &>(endZone.GetObj("sizeX")).Get());
      endSize.second = stod(
	      dynamic_cast<JSON::JsonStr &>(endZone.GetObj("sizeY")).Get());

      JSON::JsonObj &spawn = dynamic_cast<JSON::JsonObj &>(json.GetObj(
	      "spawn"));
      spawnPoint.first = stod(
	      dynamic_cast<JSON::JsonStr &>(spawn.GetObj("x")).Get());
      spawnPoint.second = sizeY -
			  stod(dynamic_cast<JSON::JsonStr &>(spawn.GetObj(
				  "y")).Get());
	...
	throw (std::runtime_error("Spawn point coordinates are out of bounds"));
      ...
      JSON::JsonArr &arr = dynamic_cast<JSON::JsonArr &>(json.GetObj(
	      "dynamic"));
      for (unsigned int i = 0; i < arr.Size(); ++i)
	{
      ...
      JSON::JsonArr &sounds = dynamic_cast<JSON::JsonArr &>(json.GetObj(
	      "sounds"));
      for (unsigned int i = 0; i < sounds.Size(); ++i)
	{
	  ...
	  putSound(stoi(dynamic_cast<JSON::JsonStr &>(obj.GetObj("id")).Get()),
		   std::pair<double, double>
			   (stod(dynamic_cast<JSON::JsonStr &>(obj.GetObj(
				   "x")).Get()),
			    sizeY -
			    stod(dynamic_cast<JSON::JsonStr &>(obj.GetObj(
				    "y")).Get())));
