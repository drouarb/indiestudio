...
bool OgreUI::playSound(int id, gauntlet::SoundName name, bool loop) {
    ...
    if (!mSoundManager->hasSound(ss.str())) if (loadSound(id, name) == false) {
...
int OgreUI::triggerEffect(int id, gauntlet::EffectName type,
    ...
    effect->getParticleSystem()->getEmitter(0)->setDirection(
            Ogre::Vector3(std::get<0>(orientation), std::get<1>(orientation), std::get<2>(orientation)));
    return 0;
}
...
void OgreUI::addCameraTracker(int id) {
    ...
    Ogre::SceneNode *s = mSceneMgr->getSceneNode(ss.str());
    rootNode = s;
    mCamera->setPosition(s->getPosition().x, mCamera->getPosition().y + 550,
                         s->getPosition().z - 700);
...
void OgreUI::load3dSound(int id, SoundName name, int x, int y) {
    ...
    if (name != SOUND_NONE) if ((sound = mSoundManager->createSound(ss.str(), soundmap.at(name)))) {
